...
%   xx = argmin ||A*x-bb||^2 + lambda*||x||_1
%
% Syntax:
...
% [ww,stat]=dalsql1n(zeros(n,1), A, bb, lambda);
%
% Copyright(c) 2009 Ryota Tomioka
% This software is distributed under the MIT license. See license.txt
%% Modified by 2011 Shigeyuki Oba on dalsql1.m

function [ww,status]=dalsql1n(ww,A,bb, lambda, varargin)
...
prob.fspec    = @(xx)((xx>0).*abs(xx)+(xx<0).*0); %% OBA
%% prob.dnorm    = @(vv)max(abs(vv));
prob.dnorm    = @(vv)max(vv); %% OBA
prob.obj      = @objdall1n; %% OBA
prob.softth   = @l1n_softth; %% OBA
...
[ww,uu,status]=dal(prob,ww,[],fA,fAT,[],lambda,opt);
